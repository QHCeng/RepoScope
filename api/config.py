...
def replace_env_placeholders(config: Union[Dict[str, Any], List[Any], str, Any]) -> Union[
    ...
    if "providers" in generator_config:
        for provider_id, provider_config in generator_config["providers"].items():
            ...
            # Fall back to default mapping based on provider_id
                ...
                # Try to import GoogleGenAIClient if needed
                if provider_id == "google":
                    try:
                        ...
                        logger.warning("GoogleGenAIClient not available. Skipping Google provider.")
                        default_map = {
                            "openai": OpenAIClient
                        }
                else:
...
def get_embedder_config():
    ...
    if embedder_type == 'google' and 'embedder_google' in configs:
        ...
        # Return default embedder config (should always exist after initialization)
...
if generator_config:
    ...
    # Set default provider configuration if not loaded from file
    configs["default_provider"] = "openai"
    configs["providers"] = {}

# Set default providers if not loaded from file
if not configs.get("providers"):
    try:
        # Try to import GoogleGenAIClient from adalflow
        try:
            from adalflow import GoogleGenAIClient
        except ImportError:
            GoogleGenAIClient = None
            logger.warning("GoogleGenAIClient not available. Google provider will not be available.")
    except Exception as e:
        GoogleGenAIClient = None
        logger.warning(f"Could not import GoogleGenAIClient: {e}")

    # Default OpenAI provider configuration
    configs["providers"]["openai"] = {
    ...
    logger.info("Using default OpenAI provider configuration with gpt-4o as default model")

    # Default Google provider configuration (if available)
    if GoogleGenAIClient:
        configs["providers"]["google"] = {
        ...
        logger.info("Using default Google provider configuration")
...
# Set default embedder configuration if not loaded from file
if "embedder" not in configs:
    # Default OpenAI embedder configuration
    configs["embedder"] = {
        "client_class": "OpenAIClient",
        "model_client": OpenAIClient,
        "model_kwargs": {
    ...
    logger.info("Using default OpenAI embedder configuration")

# Set default retriever and text_splitter if not loaded
if "retriever" not in configs:
    configs["retriever"] = {
        "top_k": 5
    }

if "text_splitter" not in configs:
