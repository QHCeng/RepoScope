        ...
        if access_token:
            ...
            if repo_type == "github":
                ...
                clone_url = urlunparse(
                    (parsed.scheme, f"oauth2:{encoded_token}@{parsed.netloc}", parsed.path, '', '', ''))
            elif repo_type == "bitbucket":
                # Format: https://x-token-auth:{token}@bitbucket.org/owner/repo.git
                clone_url = urlunparse(
                    (parsed.scheme, f"x-token-auth:{encoded_token}@{parsed.netloc}", parsed.path, '', '', ''))
...
def read_all_documents(path: str, embedder_type: str = None, is_ollama_embedder: bool = None,
    ...
    for ext in code_extensions:
        ...
        for file_path in files:
            ...
            if not should_process_file(file_path, use_inclusion_mode, included_dirs, included_files, excluded_dirs,
                                       excluded_files):
                continue
...
class DatabaseManager:
    ...
    def prepare_database(self, repo_url_or_path: str, repo_type: str = None, access_token: str = None,
        ...
        return self.prepare_db_index(embedder_type=embedder_type, excluded_dirs=excluded_dirs,
    ...
    def prepare_db_index(self, embedder_type: str = None, is_ollama_embedder: bool = None,
        ...
        logger.info(f"Reading documents from: {self.repo_paths['save_repo_dir']}")
        ...
        if not documents:
            logger.warning(f"No documents found in repository: {self.repo_paths['save_repo_dir']}")
            logger.warning("This may indicate an empty repository or all files were excluded")
            return []
        
        logger.info(f"Found {len(documents)} documents. Starting embedding generation...")
        ...
        if not transformed_docs:
            logger.error("No transformed documents with embeddings were created. This may indicate an embedding API error.")
